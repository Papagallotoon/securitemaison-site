# Sécurité Maison — site

Version française dédiée, forkée depuis le moteur multi-niches
`readyscore-affiliate-engine` (voir `~/New Aff/readyscore-affiliate-engine`).
Une seule niche ici (`config/niches/securitemaison/`), et contrairement au
moteur d'origine, le contenu réel (le questionnaire de `content/assessment.ts`,
les articles de `content/articles.ts`) est écrit directement en français plutôt
que paramétré par niche — voir "Règles à ne pas casser" plus bas pour ce que
ça change.

Chaque article correspond à un sujet déjà publié sur la chaîne YouTube
"Sécurité Maison" (mêmes produits Amazon, mêmes prix vérifiés) — le but premier
de ce site est d'avoir un lien à mettre en description de chaque vidéo.

Stack : Next.js 14 (App Router), TypeScript, Tailwind. Aucune base de données,
aucun compte, aucun email — les réponses du quiz générique vivent en
`sessionStorage` (le vrai tunnel utilisé est `/assessment`, entièrement côté
client, rien n'est envoyé nulle part).
